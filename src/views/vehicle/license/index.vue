      ...
      <el-col :xs="24" :sm="24" :md="14" :lg="14" :xl="14">
        <el-card class="box-card">
          <div slot="header" class="clearfix">
            <span>字典详情</span>
            <el-button
              v-if="checkPermission(['ADMIN','DICT_ALL','DICT_CREATE']) && this.$refs.licenseDetail && this.$refs.licenseDetail.dictName"
              class="filter-item"
              size="mini"
              style="float: right;padding: 4px 10px"
              type="primary"
              icon="el-icon-plus"
              @click="$refs.licenseDetail.$refs.form.dialog = true;$refs.licenseDetail.isAdd = true">新增</el-button>
          </div>
          <licenseDetail ref="licenseDetail"/>
        </el-card>
      </el-col>
    </el-row>
  </div>
...
import licenseDetail from '../licenseDetail/index'
import eForm from './form'
export default {
  components: { licenseDetail, eForm },
  mixins: [initData],
  data() {
    return {
      delLoading: false,
  ...
  methods: {
    ...
    beforeInit() {
      ...
      if (this.$refs.licenseDetail) {
        this.$refs.licenseDetail.data = []
        this.$refs.licenseDetail.dictName = ''
      }
      return true
    },
    subDelete(id) {
    ...
    handleCurrentChange(val) {
      if (val) {
        this.$refs.licenseDetail.dictName = val.name
        this.$refs.licenseDetail.dictId = val.id
        this.$refs.licenseDetail.init()
      }
    },
    edit(data) {
      this.isAdd = false
